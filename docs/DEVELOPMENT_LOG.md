...
This document is the full day-by-day build log for the Nifty 100 Future
Intelligence Platform, covering Sprints 1-6. For project overview, setup,
and run instructions, see the top-level [README.md](../README.md).
...
- Built src/api/main.py - FastAPI application with CORS middleware (all origins, internal use only) and a request-logging middleware that prints method, path, status code, and response time in ms for every call.
- Built src/api/db.py - shared SQLite connection helper for the API layer, mirroring src/dashboard/utils/db.py's get_connection() pattern minus the Streamlit caching decorators (FastAPI has its own request lifecycle).
...
- **API load test**: 10 concurrent `GET /api/v1/screener` requests — PASS, 3.2–3.3s total (target: 10s)
- **Dashboard profile performance** (5 tickers, target <3s each): 3/5 PASS (TCS, INFY, MARUTI). ITC and LT failed due to live logo HTTP fetch (`get_logo_data_uri()`), not DB queries — all SQLite calls consistently under 30ms. Mitigated in production by 24h caching (`@st.cache_data(ttl=86400)`); cold-start only.
- **End-to-end test**: FastAPI (8000) + Streamlit (8501) run simultaneously with no port conflicts; manually confirmed Company Profile screen renders real data end-to-end
- **SQLite indexing**: added 16 indexes (`company_id`/`year`) across all core tables — no prior indexes existed. Measured no performance change at current scale (92 companies); documented as a forward-looking investment, not a fix for today's bottleneck
- **`output/perf_notes.md`** — full write-up of all findings and recommendations

**Recommendation flagged for review:** pre-fetch/cache company logos during ETL instead of live per-session fetch, to eliminate the one real bottleneck found.